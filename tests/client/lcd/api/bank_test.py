...
def test_balance():
    result, _ = terra.bank.balance(address="terra1x46rqay4d3cssq8gxxvqz8xt6nwlz4td20k38v")
    assert(result.to_data())
    assert(result.get("ukrw").amount > 0)


def test_balance_with_pagination():
    result, _ = terra.bank.balance(address="terra1x46rqay4d3cssq8gxxvqz8xt6nwlz4td20k38v", params=pagOpt)
    assert(result.to_data())
